...
description: 详细介绍 gopls 内置 MCP Server 的安装配置与使用方法，包括 stdio 和 HTTP 两种运行模式、编辑器集成配置，以及 go_workspace、go_search、go_diagnostics 等核心工具的使用说明。
seo:
  title: 【golang/gopls】gopls-官方gopls内置mcp server的基本使用
  description: 详细介绍 gopls 内置 MCP Server 的安装配置与使用方法，包含 HTTP 和 stdio 运行模式、Trae 等 IDE 集成配置，以及 go_workspace、go_search、go_diagnostics 等核心工具的使用说明，助力 Go 语言 AI 辅助开发。
navigation: Golang
date: 2025-09-14 19:43:40
---
...
![在这里插入图片描述](./golang/1.webp){style="max-width:360px;width:100%;display:block;margin:0 auto"}
点击原始配置：
![在这里插入图片描述](./golang/2.webp){style="max-width:360px;width:100%;display:block;margin:0 auto"}
将gopls mcp监听的地址写入：
![在这里插入图片描述](./golang/3.webp){style="max-width:520px;width:100%;display:block;margin:0 auto"}
保存后即可看到可用的工具
![在这里插入图片描述](./golang/4.webp){style="max-width:360px;width:100%;display:block;margin:0 auto"}
其他编辑器类似

- 然后就可以正常使用了
![在这里插入图片描述](./golang/5.webp){style="max-width:360px;width:100%;display:block;margin:0 auto"}
