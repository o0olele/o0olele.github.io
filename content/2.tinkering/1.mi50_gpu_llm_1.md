...
description: MI50 显卡运行大模型环境搭建教程，介绍 BIOS 设置（关闭CSM、开启Above4G）、SSH 远程连接配置、AMD GPU 驱动与 ROCm 安装步骤。
seo:
  title: 【瞎折腾】mi50显卡ubuntu运行大模型开坑(一)显卡准备以及驱动安装
  description: MI50 显卡运行大模型环境搭建教程，介绍 BIOS 设置（关闭CSM、开启Above4G）、SSH 远程连接配置、AMD GPU 驱动与 ROCm 安装步骤，适合 AMD 显卡 AI 开发入门。
...
![在这里插入图片描述](./tinkering/1.webp)
- 选择`Advanced`
![在这里插入图片描述](./tinkering/2.webp)
- 将`CSM`设置位`Disabled`，这里可能会遇到需要将`UEFI Video`设置为`Disabled`的提示，按照提示设置一下
![在这里插入图片描述](./tinkering/3.webp)
...
![在这里插入图片描述](./tinkering/5.webp)
- 找一个适用于22.04的
 ![在这里插入图片描述](./tinkering/6.webp)
...
- 安装
	...
	~~~
	![在这里插入图片描述](./tinkering/8.webp)
	![在这里插入图片描述](./tinkering/9.webp)
	等待...
	![在这里插入图片描述](./tinkering/10.webp)
