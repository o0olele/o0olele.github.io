...
description: 使用 Langchain 构建命令行聊天机器人，介绍消息历史管理、LangGraph 持久化层（MemorySaver）、StateGraph 状态管理以及历史对话裁剪（trim_messages）优化方法。
seo:
  title: 【llm/langchain】langchain入门-使用langchain编写一个简单的聊天机器人(DeepSeek API&命令行版本)
  description: 使用 Langchain 构建命令行聊天机器人，介绍消息历史管理、LangGraph 持久化层（MemorySaver）、StateGraph 状态管理以及历史对话裁剪（trim_messages）优化方法，实现有记忆的 AI 对话功能。
navigation: LLM
---
